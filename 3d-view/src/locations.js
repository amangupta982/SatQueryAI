...
export const CITY_POIS = {
  ...
  india: {
    name: 'India',
    groundElevation: 170,
    viewBounds: { southwest: { lat: 8.07, lng: 68.11 }, northeast: { lat: 35.51, lng: 97.40 } },
    pois: [
      { name: 'Taj Mahal', lat: 27.1751, lon: 78.0421, alt: 600, pitch: -25, heading: 0, buildingHeight: 73, buildingBounds: { height: 73, width: 95, depth: 95 } },
      { name: 'India Gate', lat: 28.6129, lon: 77.2295, alt: 500, pitch: -22, heading: 260, buildingHeight: 42, buildingBounds: { height: 42, width: 35, depth: 25 } },
      { name: 'Gateway of India', lat: 18.9220, lon: 72.8347, alt: 500, pitch: -22, heading: 295, buildingHeight: 26, buildingBounds: { height: 26, width: 20, depth: 15 } },
      { name: 'Statue of Unity', lat: 21.8380, lon: 73.7191, alt: 850, pitch: -20, heading: 140, buildingHeight: 182, buildingBounds: { height: 182, width: 60, depth: 40 } },
      { name: 'Red Fort', lat: 28.6562, lon: 77.2410, alt: 750, pitch: -28, heading: 270, buildingHeight: 33, buildingBounds: { height: 33, width: 120, depth: 80 } },
    ],
  },
};
 ...
 * Returns { lat, lng, label, types, viewport } or null on miss.
 * @private
 */
async function _nominatimGeocode(query) {
  ...
  // Map Nominatim class/type to Google-like types for navigation mode detection
  const types = [];
  if (r.type === 'aerodrome' || r.class === 'aeroway') types.push('airport');
  if (r.type === 'country' || r.class === 'boundary') types.push('country');
  if (r.type === 'city' || r.type === 'town' || r.type === 'village') types.push('locality');
  if (r.type === 'state' || r.type === 'administrative') types.push('administrative_area_level_1');
  if (types.length === 0) types.push('point_of_interest', 'establishment');

  // Build viewport from bounding box [south, north, west, east]
  let viewport = null;
  ...
  return { lat, lng, label: r.display_name, types, viewport };
}
 ...
 *
 * When no Google Maps API key is available, falls back to Nominatim
 * (OpenStreetMap) for free geocoding — airports, cities, landmarks all work.
 */
export async function searchAndFlyTo(viewer, query, options = {}) {
  ...
  let lat, lng, label, types, viewport;

  if (apiKey) {
    // --- Google Geocoding path (existing) ---
    ...
    const nom = await _nominatimGeocode(query);
    if (!nom) return null;
    lat = nom.lat;
    lng = nom.lng;
    label = nom.label;
    types = nom.types;
    viewport = nom.viewport;
  }
