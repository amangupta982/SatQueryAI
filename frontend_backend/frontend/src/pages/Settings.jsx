...
function Toggle({ enabled, onChange }) {
  return (
    <button
      ...
      className={`w-9 h-5 rounded-full relative transition-colors shrink-0 ${enabled ? 'bg-cyan-accent' : 'bg-slate-200'}`}
    >
      <span
...
export default function Settings() {
  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 lg:p-6 max-w-xl space-y-5 bg-[#f8f9fb]">
        <div className="glass rounded-xl p-4 flex items-center gap-3.5 bg-white border border-slate-200 shadow-sm">
          <div className="w-12 h-12 rounded-full bg-sky-100 border border-sky-300 flex items-center justify-center text-sm font-semibold text-sky-700">
        ...
        <div className="glass rounded-xl divide-y divide-slate-100 bg-white border border-slate-200 shadow-sm">
